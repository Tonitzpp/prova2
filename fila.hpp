...
template <class T>
Fila<T>::Fila(size_t capacidade) : capacidade(capacidade) {
...
void Fila<T>::enfileirar(const T& valor) {
  if (esta_cheia()) throw std::runtime_error("Fila está cheia");
...
T Fila<T>::desenfileirar() {
 if(esta_vazia()) throw std::runtime_error("A fila já está vazia");
